...
def type_go_code(code_path):
    """
    模拟人工逐字符输入 Go 语言代码到 LeetCode 编辑器

    参数:
        code_path: Go 代码文件路径 (如 'go_code.txt')

    核心处理逻辑:
    1. 花括号: 代码结构中的 '}' 不输入（IDE 自动补全），但需移动光标
    2. 缩进: 信任 IDE 自动缩进，只在缩进回退时修正
    3. 字符/字符串字面量: 其中的 '{' '(' '[' 原样输入，但需删除 IDE 自动补全的对应右括号
    4. 数组/切片初始化: {} 必须在同一行内完成

    限制说明:
    - 不支持嵌套复合字面量跨多行（如嵌套的结构体初始化）
    - 代码中不应包含注释
    """
