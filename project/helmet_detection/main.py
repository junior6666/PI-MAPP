...
class HelmetDetectionUI(QMainWindow):
    ...
    def init_ui(self):
        """初始化UI"""
        self.setWindowTitle("基于YOLO的通用目标检测系统")
        ...
        source_layout.addWidget(QLabel("选择权重:"))
        self.pt_combo = QComboBox()
        self.pt_combo.currentTextChanged.connect(self.on_model_changed)
        self.init_model_combo()
        source_layout.addWidget(self.pt_combo)
        ...
        self.select_file_btn = QPushButton("点击加载")
        ...
        self.current_model_path = Path("pt_models/helmet_best.pt")
        self.chose_model_path = None
    ...
    def load_model(self, model_path: Path):
        ...
        try:
            ...
            self.log_message(f"模型加载成功: {model_path.name}")
            ...
            self.log_message(f"错误: 模型加载失败 - {e}")
            self.model = None

    def on_model_changed(self, model_text: str):
        """模型下拉框切换时自动重新加载"""
        new_path = Path("pt_models") / model_text
        self.load_model(new_path)

    def init_model_combo(self):
        """初始化模型选择下拉框（动态加载 pt_models/*.pt）"""
        self.pt_combo = QComboBox()
        ...
        if not pt_files:
            # 目录为空时给一个占位项，避免 combo 空白
            self.pt_combo.addItem("无可用模型")
            self.pt_combo.setEnabled(False)
            self.current_model_path = None
        else:
            # 只把文件名填到 combo（显示更简洁）
            self.pt_combo.addItems([f.name for f in pt_files])
            self.pt_combo.setEnabled(True)
            # 默认选第 1 个
            self.current_model_path = pt_files[0]
    ...
    def on_model_changed(self, model_text: str):
        """模型选择改变（动态路径）"""
        model_dir = Path("pt_models")
        self.current_model_path = model_dir / model_text  # 直接拼接即可

        # 其余逻辑保持不变
        self.chose_model_path = model_text
        ...
        if self.current_source_type in ["image", "video"]:
            self.select_file_btn.setEnabled(True)
            self.start_btn.setEnabled(False)
            self.statusBar().showMessage(f"已选择模型 {model_text} - 请选择{self.current_source_type}文件")
        elif self.current_source_type == "camera":
            self.select_file_btn.setEnabled(False)
            if self.current_model_path.exists():
                self.start_btn.setEnabled(True)
                self.statusBar().showMessage(f"已选择模型 {model_text} - 可以开始摄像头检测")
            else:
                self.start_btn.setEnabled(False)
                self.statusBar().showMessage(f"模型文件不存在：{self.current_model_path}")
    ...
    def create_icon(self, size: int = 64) -> "QIcon":
        """
        为目标检测应用生成通用图标。
        图案：圆角矩形外框 + 十字准星。
        颜色：主色 #2196F3，背景白色。
        支持 HiDPI：一次性生成多种尺寸，Qt 会自动选择最合适的。
        """
        ...
        for s in standard_sizes:
            ...
            margin = max(3, s // 10)  # 边距随尺寸自适应
            side = s - 2 * margin
            rect = QRectF(margin, margin, side, side)

            # 外框：圆角矩形
            pen_width = max(1, s // 32)
